...
if [ $ret -eq 0 ] ; then
	...
	cp -r /usr/lib/kusanagi-wp/* /home/kusanagi/$PROFILE/DocumentRoot
fi
...
#cp -rp /usr/lib/kusanagi/resource/DocumentRoot/* /home/kusanagi/$PROFILE/DocumentRoot/
cp -p /usr/lib/kusanagi/resource/DocumentRoot/.htaccess /home/kusanagi/$PROFILE/DocumentRoot/
cp -rp /usr/lib/kusanagi/resource/settings /home/kusanagi/$PROFILE/
cp -rp /usr/lib/kusanagi/resource/tools /home/kusanagi/$PROFILE/
	...
	if [ $? -eq 0 ]  ; then
		...
		if [ $? -eq 0 ] ; then
			...
			if [ -d $PLUGIN_NAME ] ; then
				mv $PLUGIN_NAME /home/kusanagi/$PROFILE/DocumentRoot/wp-content/plugins
			else
				PLUGIN_VER=
			fi
			rm $WORKFILE
...
if [ $OPT_WOO ] ; then
	...
	if [ -n $WOOCOMMERCE_VERSION ] ; then
		...
		KUSANAGI_DEFAULT_INI=/home/kusanagi/$PROFILE/settings/kusanagi-default.ini
		...
		if [ $? -eq 0 ] ; then
			...
			if [ $? -eq 0 ] ; then
				...
				unzip -q $SF_ZIP
				rm $SF_ZIP
				mv storefront /home/kusanagi/$PROFILE/DocumentRoot/wp-content/themes/
		...
		if [  'ja' = $WPLANG  ]; then
			...
			if [ $? -eq 0 ] ; then
				...
				unzip -q ja.zip
				rm ja.zip
				mv ./* /home/kusanagi/$PROFILE/DocumentRoot/wp-content/languages/plugins
				cd
				rmdir $WORK
			...
			if [ -e $GMOPLUGIN ] ; then
				...
				cd $WORK
				unzip -q $GMOPLUGIN
				mv wc4jp-gmo-pg /home/kusanagi/$PROFILE/DocumentRoot/wp-content/plugins/
				# rm -rf __MACOSX
				cd
				rm -rf $WORK
